...
MVP-03 is active. Its one-peer local WebRTC page forwards browser microphone audio to VB-CABLE and exposes mute/end/test controls. The return path now uses Windows process-loopback capture scoped to the unique current-session Codex process tree; live activation and owned-child cleanup passed on the reference PC, and the 11-test browser suite includes two-way WebRTC negotiation. No endpoint-wide fallback or audio persistence exists. One live browser-to-Codex-to-browser conversation remains before local acceptance; trusted phone HTTPS, pairing, and reconnect follow without exposing the unauthenticated localhost spike.
