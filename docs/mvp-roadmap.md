...
MVP-03 is paused at local live validation. Its one-peer local WebRTC page forwards browser microphone audio to VB-CABLE and exposes mute/end/test controls, microphone selection, local activity feedback, and bounded peer/browser-audio/CABLE status. The owned host remains available while Hermes is still confirming Codex Voice and stops only on an explicit stop request. The return path uses Windows process-loopback capture scoped to the unique current-session Codex process tree. On the reference PC, 89 Python tests pass in the Hermes runtime, the native capture route activates and cleans up, and installed plugin version 0.5.0 passed a start/status/pause/status/stop lifecycle check. No endpoint-wide fallback or audio persistence exists. One successful live browser-to-Codex-to-browser conversation remains before local acceptance; trusted phone HTTPS, pairing, and reconnect follow without exposing the unauthenticated localhost spike. Resume from the bounded procedure in [the pause handoff](pause-handoff-2026-09-02.md), not from the superseded CP-013 feasibility path.
