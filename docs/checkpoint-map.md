...
> [!IMPORTANT]
> **This map no longer gates the first working prototype.** MVP-01 and MVP-02 are accepted on the reference PC. As of 2026-09-04, [MVP-D01](mvp-d01-discord-voice.md) is next: direct Discord audio with no transcription. Browser acceptance is parked, and Discord no longer waits for CP-163. Exact App Server resume (CP-013), exhaustive evidence harnesses, provider abstraction, and broad compatibility remain later hardening work. [ADR 0004](adr/0004-discord-first-direct-audio.md) records the owner's transport pivot.
...
**Status:** In progress — paginated existing-task resume unavailable; Stage C packet accepted, live approval required
...
**CP-013 Stage A, existing-task route discovery, and Stage C preparation are accepted.** Existing paginated-task resume is unavailable without an unsafe title-only fallback. The replacement fresh-task route passed 126 synthetic checks and uses collision-checked owned names, selection-item-only UIA, fresh selected-state verification, and owned archive cleanup. No live fresh task or Voice action has occurred; Stage C awaits explicit approval.

The next executable Phase Zero checkpoints are:

1. **CP-013 Stage C — Fresh task create/open/verify** — packet accepted; explicit live mutation approval required
