...
Companion tools own preflight, packaged Codex launch, the configured physical-microphone-to-VB-CABLE stream, session state, and task-preserving cleanup. `computer_use` owns the visible Codex controls needed at runtime: new or recent conversation, model, reasoning effort, Voice start, visible ready check, and Voice stop. `CABLE Output (VB-Audio Virtual Cable)` is selected once in Codex Settings during setup; it is not selected inside each Voice call.

## Tools

- `codex_voice_start`: requires `mode`. Preflights the unlocked Windows session, launches Codex if needed, proves VB-CABLE exists, starts the configured physical microphone stream into `CABLE Input (VB-Audio Virtual Cable)`, and returns `starting`. It does not click New task or Voice. Never pass task IDs, titles, or App Server resume keys.
...
- `codex_voice_stop`: cooperatively stops the audio stream and clears companion state after `computer_use` ended Voice and a post-action capture showed Voice ended. Leaves the Codex task open. Never delete, cancel, archive, or close the task.
...
1. `codex_voice_start` with `mode="new"` until status is `starting`; this also starts the configured physical microphone stream.
...
7. Tell the user Voice is ready only after confirm returns `ready`.
...
2. `codex_voice_stop` to stop the audio stream and clear companion state.
